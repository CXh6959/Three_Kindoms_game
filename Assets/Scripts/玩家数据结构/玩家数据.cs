...
namespace 玩家数据结构
{
	public class 玩家数据
	{
		...
		public int 获取指定名字将领数量(string 将领名字)
		{
			int 数量 = 0;
			for (int i = 0; i < 封地信息表.Count; i++)
			{
				if (封地信息表[i] == null || 封地信息表[i].将领信息表 == null)
				{
					continue;
				}
				for (int j = 0; j < 封地信息表[i].将领信息表.Count; j++)
				{
					将领信息 将领 = 封地信息表[i].将领信息表[j];
					if (将领 != null && 将领.将领属性 != null && 将领.将领属性.初始属性 != null && 将领.将领属性.初始属性.名字 == 将领名字)
					{
						数量++;
					}
				}
		...
		public int 消耗指定名字将领(string 将领名字, int 消耗数量)
		{
			if (消耗数量 <= 0)
			{
				return 0;
			...
			int 已消耗 = 0;
			for (int i = 0; i < 封地信息表.Count && 已消耗 < 消耗数量; i++)
			{
				List<将领信息> 列表 = 封地信息表[i] == null ? null : 封地信息表[i].将领信息表;
				if (列表 == null)
				{
					continue;
				}
				for (int j = 列表.Count - 1; j >= 0 && 已消耗 < 消耗数量; j--)
				{
					将领信息 将领 = 列表[j];
					if (将领 != null && 将领.将领属性 != null && 将领.将领属性.初始属性 != null && 将领.将领属性.初始属性.名字 == 将领名字)
					{
		...
		public void 添加指定国家全部名将(string 国家名)
		{
			...
			for (int j = 0; j < count; j++)
			{
				...
				if (将领属性库类 != null && 全局将领库.是否解锁系列将(将领属性库类))
				{
					将领属性库类.获取随机属性();
					将领信息 将领信息2 = new 将领信息();
					将领信息2.生成将领数据(将领属性库类);
			...
			for (int k = 0; k < count; k++)
			{
				...
				if (将领属性库类2 != null && 全局将领库.是否解锁系列将(将领属性库类2))
				{
					将领属性库类2.获取随机属性();
					将领信息 将领信息3 = new 将领信息();
			...
			for (int l = 0; l < count; l++)
			{
				...
				if (将领属性库类3 != null && 全局将领库.是否解锁系列将(将领属性库类3))
				{
					将领属性库类3.获取随机属性();
					将领信息 将领信息4 = new 将领信息();
